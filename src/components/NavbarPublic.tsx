...
export function NavbarPublic({ transparent = false, isScrolled = false, activePage }: NavbarPublicProps) {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      // Kedua ref bisa terpasang sekaligus (desktop hidden tapi masih di DOM); tutup hanya jika klik di luar keduanya.
    ...
    <nav className={navClass}>
      <div className="max-w-[1200px] mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-6">
          {loading ? (
            ...
            <div className="relative z-30 isolate" ref={dropdownRef}>
              <div 
              ...
              {isProfileDropdownOpen && (
                <div className="absolute top-full right-0 mt-2 w-48 bg-white border border-neutral-200 rounded-lg shadow-lg py-1 z-[100]">
                  <Link
                    href="/profile"
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden absolute top-full left-0 right-0 bg-white border-b border-neutral-100 shadow-lg py-4 px-6 flex flex-col gap-4">
          ...
          {user ? (
            <div className="relative z-30 isolate" ref={mobileDropdownRef}>
              <div 
              ...
              >
                ...
                <span className="text-sm font-medium text-neutral-800">{getDisplayName(user)}</span>
              </div>
              
              {isProfileDropdownOpen && (
                <div className="absolute top-full left-0 w-full bg-white border border-neutral-200 rounded-lg shadow-sm py-1 z-[100]">
                  <Link
                    href="/profile"
                    className="block w-full text-left px-4 py-2 text-sm text-neutral-700 hover:bg-neutral-50 transition-colors"
                    onClick={(e) => {
                  ...
                  <button type="button" onClick={() => { handleLogout(); setIsMobileMenuOpen(false); }} className="w-full text-left px-4 py-2 text-sm text-red-600 hover:bg-neutral-50 transition-colors">
                    Logout
                  </button>
                </div>
              )}
